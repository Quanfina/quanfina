"""
P508 (18 Haz 2026): /api/stock/{symbol}/blue-sky endpoint pytest.

FastAPI TestClient + smoke + shape + Carr canon tutarlilik koruma.
quanfina_math.compute_blue_sky_breakout (P507, test_blue_sky.py 14/14) backend wire.
"""
import pytest
import sys
...
def _get(client, symbol):
    return client.get(f"/api/stock/{symbol}/blue-sky")


class TestBlueSkyEndpoint:
    FIELDS = (
        "detected", "direction", "quality", "signal_close", "entry", "stop",
        "target", "risk_pct", "rr", "high_40d", "high_260d", "low_260d",
    ...
    def test_canon_consistency(self, client):
        ...
        if data["detected"]:
            ...
            assert data["risk_pct"] <= 8.01, "Carr s.325 %8 hard cap ihlali"
            expected = data["entry"] + 2.0 * (data["entry"] - data["stop"])
            assert abs(data["target"] - expected) < 0.1
            # 52h zirve ALTI + 40g yeni yuksek (Blue Sky tanimi)
            assert data["signal_close"] < data["high_260d"]
        else:
